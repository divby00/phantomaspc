...
static int read_ogg_data(struct OggVorbis_File *vf, char *buffer, int length, int word, int sgned, int *bitstream) {
    int ret;
    TRACE("read_ogg_data: read %d bytes\n", length);
    ret = ov_read(vf, buffer, length,
...
static ov_callbacks file_callbacks = {&file_read, &file_seek, &file_close, &file_tell};
...
static ov_callbacks packfile_callbacks = {&packfile_read, &packfile_seek, &packfile_close, &packfile_tell};
...
static int memory_seek(void *datasource, ogg_int64_t offset, int whence) {
    ...
    switch (whence) {
        case SEEK_SET:
            ...
            break;
        case SEEK_CUR:
            position = ovd->position + offset;
            break;
        case SEEK_END:
            position = ovd->size + offset;
            break;
        default:
            ASSERT(0);
            break;
...
static ov_callbacks memory_callbacks = {&memory_read, &memory_seek, &memory_close, &memory_tell};
...
static ov_callbacks datafile_callbacks = {&memory_read, &memory_seek, &memory_close_free, &memory_tell};
...
static SAMPLE *load_ov(OggVorbis_File *vf) {
    ...
    TRACE("ov_pcm_total says %ld samples\n", num_samples);
    if (num_samples < 0) {
        TRACE("ov_pcm_total failed: %ld - not fatal\n", (ogg_int64_t) num_samples);
    }
    TRACE("ov_load: create sample: %d bits, %d channels (%s), %ld Hz, %ld samples\n", 16, info->channels,
          stereo ? "stereo" : "mono", info->rate, num_samples);
    sample = create_sample(16, stereo, info->rate, num_samples > 0 ? num_samples : 1);
    if (!sample) {
        TRACE("Failed creating a sample (%d channels, %ld Hz, %zu samples", info->channels, info->rate,
              (size_t) num_samples);
        goto load_error_file;
    }
    ...
    for (offset = 0, buffer_size = 0;;) {
        if (num_samples < 0) {
            ...
            if (offset + max_read > buffer_size) {
                TRACE("Buffer too small: adding %zu bytes\n", buffer_inc);
...
int aloggint_get_info(datogg_object *ovd, int *channels, int *freq,
#ifdef ALOGG_USE_TREMOR
        ogg_int64_t *length
#else
                      float *length
...
alogg_stream *alogg_start_streaming(AL_CONST char *filename, size_t block_size) {
    ...
    stream = alogg_start_streaming_callbacks(f, (struct ov_callbacks *) &packfile_callbacks, block_size, NULL);
    if (!stream) pack_fclose(f);
    return stream;
}

alogg_stream *alogg_start_streaming_datafile(AL_CONST DATAFILE *dat, size_t block_size) {
    alogg_stream *stream = NULL;
    ...
    ovd->allocated = 0;
    ovd->data = dat->dat;
    stream = alogg_start_streaming_callbacks(ovd, (struct ov_callbacks *) &datafile_callbacks, block_size, NULL);
    if (!stream) free(ovd);
    return stream;
}

alogg_stream *aloggint_start_streaming_datogg(datogg_object *ovd, size_t block_size) {
    if (!ovd) return NULL;
    ovd->position = 0;
    return alogg_start_streaming_callbacks(ovd, (struct ov_callbacks *) &memory_callbacks, block_size, NULL);
}
...
alogg_stream *alogg_start_streaming_callbacks(void *datasource, struct ov_callbacks *callbacks, size_t block_size,
    ...
    stream->audio_stream = play_audio_stream(stream->block_size / 2 / (stereo ? 2 : 1), /* 16 bits per sample */
...
int alogg_read_stream_data(alogg_stream *stream, void *block, size_t size) {
    ...
    TRACE("Reading stream data: %zu bytes at %p\n", size, block);
    /* decode the encoded data */
    for (; size - read_size > 0;) {
        int ret = read_ogg_data(stream->vf, ((char *) block) + read_size, size - read_size, 2, 0, NULL);
        if (ret == 0) {
            TRACE("read_ogg_data returned zero\n");
            break;
        }
        if (ret < 0) {
            TRACE("read_ogg_data returned %d, read size %zu\n", ret, read_size);
            if (read_size) break;
            TRACE("read_ogg_data failed: %d\n", ret);
            alogg_error_code = ret;
            return ret;
...
static int write_vorbis_headers(alogg_encoding_data *data, size_t ncomments, char **user_comments) {
...
static int encode_buffers_8(float **buffers, AL_CONST void *data, size_t num_samples, size_t channels) {
    size_t n, c;
...
static int encode_buffers_16(float **buffers, AL_CONST void *data, size_t num_samples, size_t channels) {
    size_t n, c;
...
static int encode_buffers(float **buffers, AL_CONST void *data, size_t num_samples, size_t channels, size_t bits) {
    int ret = 0;
    switch (bits) {
        case 8:
        ...
        case 16:
            ...
            break;
        default:
            TRACE("Unsupported sample size: %zu\n", bits);
            ret = 0;
            break;
    }
    return ret;
...
int
alogg_save_ogg_param(AL_CONST char *filename, struct SAMPLE *sample, float quality, size_t ncomments, char **comments) {
    ...
    data = alogg_start_encoding(sample->stereo ? 2 : 1, sample->freq, quality, ncomments, comments,
                                (void (*)(void *, size_t, unsigned long)) &pack_fwrite, (unsigned long) f);
    ret = !pack_ferror(f);
    if (data) {
        ret = alogg_update_encoding(data, sample->data, sample->len, sample->stereo ? 2 : 1, sample->bits) && ret;
...
alogg_encoding_data *
alogg_start_encoding(size_t channels, size_t freq, float quality, size_t ncomments, char **comments,
...
int alogg_update_encoding(alogg_encoding_data *data, AL_CONST void *buffer, size_t num_samples, size_t channels,
                          size_t bits) {
    float **buffers;
    int ret;
    int eos = 0;
